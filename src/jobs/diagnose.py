...
from ..code_context.manager import resolve_code_roots
...
async def _diagnose_with_adapter(
    bundle: DiagnosticBundle,
    prompt_template: str,
) -> tuple[DiagnosisResult, list]:
    adapter = get_adapter()
    code_roots = resolve_code_roots(bundle)
...
async def run_diagnosis(ticket_id: str, bundle: DiagnosticBundle) -> None:
    ...
    for attempt in range(DIAGNOSIS_MAX_RETRIES + 1):
        try:
            result, code_roots = await _diagnose_with_adapter(bundle, prompt_template)
